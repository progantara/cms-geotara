...
import data from "./wisataData.js";
import swal from "sweetalert";

const WisataList = () => {
  const sort = 3;
  let paggination = Array(Math.ceil(data.wisataData.data.length / sort))
    .fill()
    .map((_, i) => i + 1);

  const activePag = useRef(0);
  const jobData = useRef(
    data.wisataData.data.slice(
  ...
  const onClick = (i) => {
    ...
    jobData.current = data.wisataData.data.slice(
      activePag.current * sort,
      (activePag.current + 1) * sort
    );
    /* setdemo(
          ...
          <div className="w-100 table-responsive">
            <div id="example_wrapper" className="dataTables_wrapper">
              <table id="example" className="display w-100 dataTable">
                <thead>
                  <tr role="row">
                    {data.wisataData.columns.map((d, i) => (
                      <th key={i}>{d}</th>
                    ))}
                  </tr>
                </thead>
